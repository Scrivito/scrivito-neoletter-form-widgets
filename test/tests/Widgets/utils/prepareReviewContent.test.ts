...
jest.mock('lodash-es', () => ({
    ...
    isEmpty: jest.fn().mockImplementation((value) => value === null || value === undefined || value == ""),
}));
...
describe('prepareReviewContent', () => {
    ...
    it('should prepare review content with includeEmptyAnswers: false', () => {
        ...
        const hiddenInput = reviewContent[2].find(item => item.title === 'hidden input');
        expect(hiddenInput).toEqual({ title: 'hidden input', value: 'i am hidden but included' });

        const label1 = reviewContent[1].find(item => item.title === 'Label 1');
        expect(label1).toEqual({ title: 'Label 1', value: 'on' });

        const secondDate = reviewContent[2].find(item => item.title === 'second date');
