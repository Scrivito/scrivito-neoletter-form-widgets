...
describe('FormInputFieldWidget Component', () => {
  ...
  it('renders input field with email type, not required & no helptext', () => {
    pageRenderer.render({
      body: [new FormInputFieldWidget({ ...widgetProps, type: "email", required: false, helpText: "" })],
    });
  ...
  it('renders input field with telephone type, required & with help text', () => {
    pageRenderer.render({
      body: [new FormInputFieldWidget({ ...widgetProps, type: "phone_number", required: true, placeholder: 'Another Test placeholder' })],
    });
    const label = screen.getByText(widgetProps.label);
    const mandatoryText = document.querySelector(".text-mandatory");
    const helpTextElement = document.querySelector('i');
    const inputElement = screen.getByPlaceholderText('Another Test placeholder');
  ...
  it('renders correctly', () => {
    const tree = pageRenderer.getAsJSON({ body: [new FormInputFieldWidget(widgetProps)] })
