...
describe('FormSelectWidget', () => {
  ...
  it('handles selection change in the select list', () => {
    ...
    const radioInput = document.querySelector('[type="radio"][value="Item 2"]')!;
  ...
  it('renders FormSelectWidget with checkboxes and inlineView', () => {
    const selectProps = { ...widgetProps, inlineView: true, selectionType: 'multi', required: false };
  ...
  it('renders FormSelectWidget with linear-scale selection', () => {
    ...
    lodashEsModule.map.mockImplementation((array: number[], callback: (value: number, index: number) => string) => array.map(callback));
    ...
    const linearScaleContainer = document.querySelector('.linear-scale-container');
  ...
  it('renders correctly', () => {
    const tree = pageRenderer.getAsJSON({ body: [new FormSelectWidget(widgetProps)] })
