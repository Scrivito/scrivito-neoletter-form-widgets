...
const conditions = [
    new FormConditionWidget({ title: 'select me', content: [new FormDateWidget({ customFieldName: "custom_date", title: "date", dateType: "date" })], }),
    new FormConditionWidget({ title: 'i am better :)', content: [new FormDateWidget({ customFieldName: "custom_date2", title: "date", dateType: "date" })], }),
];
const widgetProps = {
    headerType: "radio",
...
describe("FormConditionalContainerWidget", () => {
    ...
    it('should show header-info & condition-info if isInPlaceEditingActive is true', () => {
        ...
        const condtionalContentInfo = document.querySelector('.conditional-content');
    ...
    it('should have radios as header', () => {
        ...
        pageRenderer.render({
            body: [new FormConditionalContainerWidget({
    ...
    it('should have dropdown as header', () => {
        ...
        pageRenderer.render({
            body: [new FormConditionalContainerWidget({
                ...widgetProps, headerType: 'dropdown', conditions: [
                    // reassign is necessary, otherwise conditions will be empty
                    new FormConditionWidget({ title: 'select me', content: [new FormDateWidget({ customFieldName: "custom_date", title: "date", dateType: "date" })], }),
                    new FormConditionWidget({ title: 'i am better :)', content: [new FormDateWidget({ customFieldName: "custom_date2", title: "date", dateType: "date" })], }),
                ]
    ...
    it('should select the second option in the dropdown and show the corresponding condition for it', () => {
        ...
        const { container } = pageRenderer.render({
            body: [new FormConditionalContainerWidget({
                ...widgetProps, headerType: 'dropdown', conditions: [
                    new FormConditionWidget({ title: 'select me', content: [new FormDateWidget({ customFieldName: "custom_date_first_condition", title: "date", dateType: "date" })], }),
                    new FormConditionWidget({ title: 'i am better :)', content: [new FormDateWidget({ customFieldName: "custom_date_second_condtion", title: "date", dateType: "date" })], }),
                ]
            })]
        });
        ...
        const unSelectedCondition = container.querySelector('input[name="custom_date_first_condtion"]');
        const selectedCondition = container.querySelector('input[name="custom_date_second_condtion"]');
